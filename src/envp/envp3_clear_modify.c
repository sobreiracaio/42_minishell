...
bool	env_mod(t_shell *shell, char *target, char *new_value)
{
    ...
    while (tmp) // Iterate through each environment variable in the linked list.
    {
        if (ft_strcmp(target, tmp->key) == 0) // If the key of the current variable matches the target key...
        {
            ...
            tmp->value = malloc(ft_strlen(new_value) + 1); // Allocate memory for the new value.
            if (tmp->value) // If the memory allocation was successful...
            {
                ft_strlcpy(tmp->value, new_value, ft_strlen(new_value) + 1); // Copy the new value to the allocated memory.
