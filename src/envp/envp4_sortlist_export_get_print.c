...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   envp4_strtok_export_print_get.c                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: joaosilva <joaosilva@student.42.fr>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/29 14:28:33 by joaosilva         #+#    #+#             */
/*   Updated: 2024/05/22 10:43:02 by joaosilva        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	envp_print(t_shell *shell)
{
	...
	tmp = shell->env_list; // Set a temporary pointer to the beginning of the shell's environment linked list.
	while (tmp) // Iterate through each environment variable in the linked list.
	{
		if (tmp->visible) // If the environment variable is marked as visible...
			ft_printf("%s=%s\n", tmp->key, tmp->value); // Print the key-value pair.
...
void	env_export(t_shell *shell, char *key, char *value, int visible)
{
    if (!env_get_value(key, shell)) // If the key does not exist...
        add_node_to_envp_list(shell, key, value, visible); // Add a new variable.
    else if (env_get_value(key, shell) && visible) // If the key exists and is marked as visible...
        env_mod(shell, key, value); // Modify the value of the existing variable.
}
