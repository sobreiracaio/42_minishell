...
// Função para criar um nó. É semelhante à ft_lstnew, mas além do valor/conteúdo, tb tem o visivel, 
...
// Função semelhante à lstadd_back, para adicionar um novo nó ao final da lista ligada de variáveis de ambiente.
t_env *add_node_to_envp_list(t_shell *shell, char *key, char *value, int visible)
{
	...
	if(env_lstadd_back(&shell->env_list, new_node))
		shell->envp_size++; 
	free(new_node);
	envp_to_sort_list(shell); // Sort the environment variables alphabetically by key.	
